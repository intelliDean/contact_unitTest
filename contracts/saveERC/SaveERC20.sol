...
contract SaveERC20 {
    ...
    function deposit(uint256 _amount) external {
        ...
        require(IDean20(savingToken).balanceOf(msg.sender) >= _amount, "not enough token");

        require(IDean20(savingToken).transferFrom(msg.sender, address(this), _amount), "failed to transfer");
    ...
    function withdraw(uint256 _amount) external {
        ...
        require(IDean20(savingToken).transfer(msg.sender, _amount), "failed to withdraw");
    ...
    function checkContractBalance() external view returns(uint256) {
        return IDean20(savingToken).balanceOf(address(this));
    }

    function ownerWithdraw(uint256 _amount) external {
        require(msg.sender == owner, "not owner");

        IDean20(savingToken).transfer(msg.sender, _amount);
    }
}